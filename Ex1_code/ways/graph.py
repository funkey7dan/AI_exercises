...
# define class Link
Link = namedtuple('Link',['source','target',  #  int (junction indices)
                          'distance',  #  float
...
Junction = namedtuple('Junction',['index',  #  int
...
def load_map_from_csv(filename = 'israel.csv',start = 0,count = sys.maxsize):
    ...
    with tools.dbopen(filename,'rt') as f:
        ...
        if count < sys.maxsize:
            lst = {i: Junction(i,j.lat,j.lon,tuple(lnk for lnk in j.links if lnk.target in lst)) for i,j in lst.items()}
    return Roads(lst)
